...
export class GallerySwiper {
	...
	/** The goto callback from constructor. @internal */
	#goto:(i:number) => void;

	/** Navigate to a specific index. Calls the registered goto callback. */
	...
	constructor(
		micrio:HTMLMicrioElement,
		length:number,
		goto:(i:number) => void, // Callback to change the active index
		opts:{
			sensitivity?:number, // Multiplier for swipe distance calculation
			continuous?:boolean, // Enable continuous looping?
			coverLimit?:boolean // Only allow swiping when zoomed out fully?
