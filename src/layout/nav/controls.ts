...
micr-io.hide-ui micrio-controls aside:not(.grid-close):not(:hover) {
	transform: translateX(calc(100% + var(--micrio-border-margin)));
	opacity: 0;
	pointer-events: none;
}
micr-io[data-switching] > micrio-controls,
micr-io[data-tour-active] > micrio-controls {
	opacity: 0;
	pointer-events: none;
}
micrio-controls > micrio-button,
micrio-controls > menu {
	padding: 0;
	margin: 8px 0;
	display: block;
	width: var(--micrio-button-size);
}
micrio-controls menu.ctrl-lang {
	position: relative;
	padding: 0;
	margin: 8px 0;
	width: var(--micrio-button-size);
	min-height: var(--micrio-button-size);
}
micrio-controls .lang-items {
	position: absolute;
	right: 100%;
	top: 0;
	display: none;
...
micrio-controls menu.ctrl-lang:hover .lang-items,
micrio-controls menu.ctrl-lang:focus-within .lang-items {
	display: flex;
}
micrio-controls .lang-items micrio-button {
...
micrio-controls .lang-items .micrio-button > span {
	width: var(--micrio-button-size);
}
micrio-controls .lang-items .micrio-button.active {
	background: var(--micrio-color-hover);
	color: var(--micrio-button-background, var(--micrio-background));
}
`;
