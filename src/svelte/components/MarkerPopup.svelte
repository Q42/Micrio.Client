<script lang="ts">
	...
	/** Show the tour step counter in the popup. */
	const showTourStepCounter = $derived(isPartOfTour && !markerTour?.isSerialTour && (tourSourceImage?.$settings._markers?.tourStepCounterInPopup ?? settings.tourStepCounterInPopup));
	...
	<aside>
		...
		{#if showTourControls && $tour && 'steps' in $tour}
			...
			<ButtonGroup className="micrio-tour-controls">
				...
				{#if showTourStepCounter}<button class="micrio-button tour-step" disabled>{currentTourStep+1} / {$tour.steps.length}</button>{/if}
				<!-- Next Step Button -->
