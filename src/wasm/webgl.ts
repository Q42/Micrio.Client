...
export default class WebGL {
	...
	/**
	 * Calculates the screen coordinates of the four corners of a quad placed in 360 space.
	 * @param cX Center X of the quad (0-1 image coords).
	 * @param cY Center Y of the quad (0-1 image coords).
	 * @param w Width of the quad (relative to image).
	 * @param h Height of the quad (relative to image).
	 * @param scaleX Horizontal scale factor for the quad.
	 * @param scaleY Vertical scale factor for the quad.
	 * @param rotX Rotation around X-axis (pitch).
	 * @param rotY Rotation around Y-axis (yaw).
	 * @param rotZ Rotation around Z-axis (roll).
	 * @returns Float32Array containing screen coordinates [x0,y0,z0, x1,y1,z1, x2,y2,z2, x3,y3,z3].
	 */
	getQuad(cX:f64, cY:f64, w:f64, h:f64, scaleX:f64=1, scaleY:f64=1, rotX:f64=0, rotY:f64=0, rotZ:f64=0) : Float32Array {
		const s:f64 = PI * 2 * this.radius; // Scale factor based on sphere circumference
		const m = this.iMatrix; // Use inverse matrix for building transformation
		const pm = this.pMatrix; // Use projection matrix for final transform
		const el = this.canvas.el;
		// Get 3D coordinates of the quad center on the sphere
		const center = this.canvas.webgl.getVec3(cX, cY, true);
		const r = el.width / el.ratio; // Screen width adjusted for DPR
		// Calculate quad corner coordinates relative to center
		const x0 = cX-w/2, x1 = cX+w/2, y0 = cY-h/2, y1 = cY+h/2;
		const v = this.quad; // Target matrix to store screen coords
		const a = el.aspect; // Screen aspect ratio

		// --- Build Transformation Matrix (m) ---
		m.identity();
		m.rotateY(this.baseYaw); // Apply base yaw (true north)
		m.translate(center.x, center.y, center.z); // Translate to center point on sphere
		// Rotate to align with sphere surface normal + embed rotation Y
		m.rotateY(atan2(center.x,center.z) + PI + rotY);
		// Apply pitch based on latitude + embed rotation X
		m.rotateX(-Math.sin((cY-.5)*PI) - rotX);
		// Apply roll (embed rotation Z)
		m.rotateZ(-rotZ);
		// Apply embed scaling
		m.scaleXY(scaleX, scaleY);
		// Apply overall image scale (from parent Image object)
		...
		// Return the Float32Array view of the matrix containing screen coordinates
		return v.toArray();
	}
